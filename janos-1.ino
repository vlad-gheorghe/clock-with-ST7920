...
void draw(void) {
 ...
 u8g.drawRFrame(2,31,55,15,2);
  ...
  u8g.drawRFrame(2,47,55,15,2);
  u8g.drawRFrame(58,47,68,15,2);
...
sensors.requestTemperatures(); temperature = sensors.getTempCByIndex(0); 
 int temperatureC = temperature;
  ...
  const char* thisTemp = (const char*) thisTemp1.c_str();
   ...
   u8g.drawStr(5,60,thisTemp);
   
   float tensiune= analogRead (A0);
//float tensiune = 0;
float previousTensiune = 0;
float  tensiune0;
 float tensiune1=analogRead (A0);
 tensiune=analogRead (A0);
 //470 este valoarea rezistentei dintre mijloc si masa
 //4700 este valoarea rezistentei dintre mijloc si tensiunea de masurat
 //La mijloc se leaga A0
 float tensiuneC=(tensiune*(1.046/1023))/(462.33/(462.33+46872)); //aici masori tensiunea de referinta si inlocuiesti1.1 cu valoarea masurata 
 String tensiunec = String(tensiuneC) + " V"; 
  u8g.setFont(u8g_font_9x18B);
  const char* tensiuneV = (const char*) tensiunec.c_str();
   //u8g.setFont(u8g_font_fur17); 
   u8g.drawStr(62,60,tensiuneV);
 
// int tensiune=tensiune1;
